...
/* 把srmtk类型的作用域压入作用域栈，得到每个符号的作用域，以及是否被注册
 * 检查表达式类型是否在符号表，若不在则错误，在的话，就将行号加入给其属性
 * 如果是申明变量，则检查是否在符号表，若不在，若是函数申明，则加入符号表并压入新作用域；这里如果是变量，加入符号表
 * 若是函数参数类型，若不存在，则插入符号表
 */
...
/*检查分析树中节点信息是否正确，正确则获取该节点符号的信息并返回，如果错误则报错*/
